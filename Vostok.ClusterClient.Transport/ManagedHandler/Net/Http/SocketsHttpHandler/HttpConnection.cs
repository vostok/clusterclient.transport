...
namespace System.Net.Http
{
    internal partial class HttpConnection : HttpConnectionBase, IDisposable
    {
        ...
        {
            var encodingSelector = _pool.Settings._requestHeaderEncodingSelector;
            ...
            foreach (KeyValuePair<HeaderDescriptor, string[]> header in headers.GetHeaderDescriptorsAndValues())
            {
                ...
                if (header.Value.Length > 0)
                {
                    ...
                    await WriteStringAsync(header.Value[0], valueEncoding).ConfigureAwait(false);

                    if (cookiesFromContainer != null && header.Key.KnownHeader == KnownHeaders.Cookie)
                    {
                        await WriteTwoBytesAsync((byte)';', (byte)' ').ConfigureAwait(false);
                        await WriteStringAsync(cookiesFromContainer, valueEncoding).ConfigureAwait(false);
                    ...
                    if (header.Value.Length > 1)
                    {
                        ...
                        for (int i = 1; i < header.Value.Length; i++)
                        {
                            ...
                            await WriteStringAsync(header.Value[i], valueEncoding).ConfigureAwait(false);
                        }
                    }
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        allowExpect100ToContinue = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        private static void ParseHeaderNameValue(HttpConnection connection, ReadOnlySpan<byte> line, HttpResponseMessage response, bool isFromTrailer)
        {
            ...
            string headerValue = descriptor.GetHeaderValue(line.Slice(pos), valueEncoding);
        ...
        private async Task WriteBytesSlowAsync(byte[] bytes, int length)
        {
            int offset = 0;
            while (true)
            {
        ...
        private Task WriteStringAsync(string s, Encoding encoding)
        {
            if (encoding is null)
            {
                return WriteStringAsync(s);
            }

            // If there's enough space in the buffer to just copy all of the string's bytes, do so.
            if (encoding.GetMaxByteCount(s.Length) <= _writeBuffer.Length - _writeOffset)
            {
                _writeOffset += encoding.GetBytes(s, 0, s.Length, _writeBuffer, _writeOffset);
                return Task.CompletedTask;
            }

            // Otherwise, fall back to doing a normal slow string write
            return WriteStringWithEncodingAsyncSlow(s, encoding);
        }

        private async Task WriteStringWithEncodingAsyncSlow(string s, Encoding encoding)
        {
            ...
            try
            {
                int written = encoding.GetBytes(s, 0, s.Length, rentedBuffer, 0);
                await WriteBytesSlowAsync(rentedBuffer, written).ConfigureAwait(false);
            }
            finally
            {
