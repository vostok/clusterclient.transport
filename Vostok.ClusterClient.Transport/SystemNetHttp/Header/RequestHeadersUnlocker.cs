                    ...
                    foreach (var (name, expectedValue) in tests)
                    {
                            ...
                            !headers.TryGetValues(name, out var value))
                        {
                            log
                                ...
                                .Warn("Can't unlock HttpHeaders. Test failed on header '{name}'. Unable set header value.", name);
                        ...
                        if (!string.Equals(value.FirstOrDefault(), expectedValue, StringComparison.Ordinal))
                        {
                            log
                                .ForContext(typeof(RequestHeadersUnlocker))
                                .Warn("Can't unlock HttpHeaders. Test failed on header '{name}'. Expected value: '{expectedValue}', actual: '{value}'.", name, expectedValue, value);
                            return false;
                        }
                    }
                }
