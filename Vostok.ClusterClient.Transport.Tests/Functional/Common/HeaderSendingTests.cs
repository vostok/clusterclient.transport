...
{
    internal abstract class HeaderSendingTests<TConfig> : TransportFunctionalTests<TConfig>
        where TConfig : ITransportTestConfig, new()
    {
        ...
        [TestCase(HeaderNames.AcceptEncoding, "gzip; q=1.0, identity; q=0.5, *; q=0.0")]
        ...
        [Test]
        public void Should_correctly_transfer_a_non_ascii_header_to_server()
        {
            ...
            {
                var request = Request.Post(server.Url).WithHeader("Custom-Header", "кириллица");

                Send(request);

                server.LastRequest.Should().Contain("Custom-Header: кириллица");
            }
        }
